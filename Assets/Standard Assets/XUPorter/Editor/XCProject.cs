	...
	public partial class XCProject : System.IDisposable
	{
				...
				switch( fileReference.buildPhase ) {
					case "PBXFrameworksBuildPhase":
						...
						if ( !string.IsNullOrEmpty( absPath ) && ( tree.CompareTo( "SOURCE_ROOT" ) == 0 )) {
							string prePath = "$(SRCROOT)";
							#if UNITY_EDITOR_WIN 
							prePath = prePath + "/";
							#endif
							string libraryPath = Path.Combine( prePath, Path.GetDirectoryName( filePath ) );
		...
		{	
			...
			foreach( XCModFile libRef in mod.libs ) {
				string prePath = "usr/lib";
				#if UNITY_EDITOR_WIN 
				prePath = prePath + "/";
				#endif
				string completeLibPath = System.IO.Path.Combine( prePath, libRef.filePath );
			...
			foreach( string framework in mod.frameworks ) {
				...
				string prePath = "System/Library/Frameworks";
				#if UNITY_EDITOR_WIN 
				prePath = prePath + "/";
				#endif
				string completePath = System.IO.Path.Combine( prePath, filename[0] );
			...
			foreach( string folderPath in mod.folders ) {
				string absoluteFolderPath = System.IO.Path.Combine( mod.path, folderPath );//Application.dataPath, folderPath );
